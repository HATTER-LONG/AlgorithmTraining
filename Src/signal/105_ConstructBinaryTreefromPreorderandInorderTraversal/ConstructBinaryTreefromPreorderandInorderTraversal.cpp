...
public:
    ...
    TreeNode* buildTreeImp(const vector<int>& preorder,
        ...
        // 递归构造左子树中的节点，并链接根节点
        //  先序遍历中「从 左边界+1 开始的 size_left_subtree」个元素就对应了中序遍历中「从 左边界
        //  开始到
        /// 根节点定位-1」的元素
        root->left =
            buildTreeImp(preorder, inorder, preorder_left + 1, preorder_left + size_left_subtree,
                         inorder_left, inorder_root - 1);

        // 递归地构造右子树，并连接到根节点
        // 先序遍历中「从 左边界+1+左子树节点数目 开始到 右边界」的元素就对应了中序遍历中「从
        // 根节点定位+1 到 右边界」的元素
        root->right = buildTreeImp(preorder, inorder, preorder_left + size_left_subtree + 1,
