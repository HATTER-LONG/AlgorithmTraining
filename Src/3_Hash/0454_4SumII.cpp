...
public:
    int
    fourSumCount(vector<int>& nums1, vector<int>& nums2, vector<int>& nums3, vector<int>& nums4) {
...
TEST_CASE("Check Solution fourSumCount method work successfully") {
    ...
    tie(param, param2, param3, param4, result) =
        GENERATE(table<vector<int>, vector<int>, vector<int>, vector<int>, int>({
            make_tuple(vector<int>{1, 2}, vector<int>{-2, -1}, vector<int>{-1, 2},
                       vector<int>{0, 2}, 2),
            make_tuple(vector<int>{-1, -1}, vector<int>{-1, 1}, vector<int>{-1, 1},
                       vector<int>{1, -1}, 6),
        }));
